    ...
    for l = 1:length(y)
        for m = 1:length(x)            
            if((x(m)^2+y(l)^2)^0.5 < a)
                ...
                inc_wave(l,m,zero_idx,iter) = exp(1i*k*X(l,m,zero_idx));
            end
        end
    end
end
    ...
    for l = zero_idx:length(y)
        for n = zero_idx+1:length(z)
            ...
            for m = zero_idx:length(x)
                scatter_wave(l,m,n, iter) = scatter_wave(y_idx, m, zero_idx, iter);
                inc_wave(l,m,n,iter) = inc_wave(y_idx,m,zero_idx,iter);
            end
        end
    end
end
